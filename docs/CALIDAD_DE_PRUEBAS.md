...
[Fact]
public async Task CrearAreaConocimiento_Funciona()
{
    var creado = await servicio.CrearAreaConocimientoAsync(peticion);
    // (sin Assert: si CrearAreaConocimientoAsync guarda mal, esta prueba PASA)
}
```
...
```csharp
[Fact]
public async Task CrearAreaConocimiento_GuardaElNombre()
{
    var creado = await servicio.CrearAreaConocimientoAsync(peticion);
