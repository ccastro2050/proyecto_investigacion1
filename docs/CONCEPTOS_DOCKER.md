...
- Viene de un **registro** o se construye localmente. Este proyecto usa de
  ...
  `api_investigacion/` (base: `dotnet/sdk:10.0`).
...
tiene 2: `./api_investigacion`, `./front_blazor`.
...
WORKDIR /app
ENV DOTNET_USE_POLLING_FILE_WATCHER=1 ASPNETCORE_URLS=http://0.0.0.0:8070
EXPOSE 8070
CMD ["dotnet", "watch", "run", "--non-interactive", "--project", "ApiInvestigacion.csproj"]
```
...
| `EXPOSE 8070` | **Documenta** en qué puerto escucha el programa | No abre nada: quien publica el puerto es el `ports:` del compose |
...
| **Bind mount** | Una carpeta de SU disco montada dentro del contenedor | `./api_investigacion:/app` (el código entra al contenedor y `dotnet watch` lo vigila) · `./db:/scripts:ro` (los scripts del init, solo lectura) |
...
flowchart LR
    ...
    subgraph PC["Su PC — Docker Desktop (el 'centro de datos')"]
        subgraph RED["red interna del compose (LAN virtual, con DNS propio)"]
            FRONTBLAZOR["SERVIDOR WEB (front)<br/>contenedor front-blazor<br/>hostname: front-blazor · escucha en 8071"]
            APIINVESTIGACION["SERVIDOR DE APLICACIONES<br/>contenedor api-investigacion<br/>hostname: api-investigacion · escucha en 8070"]
    ...
    NAV -->|"localhost:8070"| APIINVESTIGACION
    NAV -->|"localhost:8071"| FRONTBLAZOR
    APIINVESTIGACION -->|"sqlserver:1433 (DNS de Docker)"| SQLSERVER
    FRONTBLAZOR -->|"http://api-investigacion:8070<br/>al controlador, por el NOMBRE"| APIINVESTIGACION
    SQLSERVERINIT -->|"espera el healthcheck,<br/>siembra y termina"| SQLSERVER
    CLIENTE -.->|"opcional (diagnóstico):<br/>localhost:11470"| SQLSERVER
```
...
```yaml
  sqlserver:
    ...
    ports:
      - "11470:1433"                 # "puerto en su PC : puerto interno"
...
```yaml
  sqlserver-init:
    ...
    volumes:
      - ./db:/scripts:ro             # init.sh + investigacion_local.sql, solo lectura
...
```yaml
  api-registros:
    build: ./api_investigacion            # se construye con SU Dockerfile
    volumes:
      - ./api_investigacion:/app          # guardar un .cs → dotnet watch recompila
...
1. **Dos redes de nombres**: hacia su PC, puertos publicados
   (`localhost:8070`, `localhost,11470`); entre contenedores, nombres de
...
docker network create proyecto_investigacion1_default

# 2. sqlserver
docker run -d --name investigacion-sqlserver --network proyecto_investigacion1_default --restart unless-stopped `
  -e "ACCEPT_EULA=Y" `
  -e "MSSQL_SA_PASSWORD=Aplicacionweb123!" `
  -v mssqldata:/var/opt/mssql `
  -p 11470:1433 mcr.microsoft.com/mssql/server:2022-latest

# 3. ESPERAR a que responda de verdad… mirándolo a ojo

# 4. sqlserver-init
docker run -d --name investigacion-sqlserver-init --network proyecto_investigacion1_default --restart no --entrypoint /bin/bash `
  -e "MSSQL_SA_PASSWORD=Aplicacionweb123!" `
  -v "${PWD}/db:/scripts:ro" mcr.microsoft.com/mssql/server:2022-latest /scripts/init.sh

# 5. Construir la imagen de api-investigacion y encenderla
docker build -t api-investigacion ./api_investigacion
docker run -d --name investigacion-api --network proyecto_investigacion1_default --restart unless-stopped `
  -e "ConnectionStrings__SqlServer=Server=sqlserver,1433;Database=investigacion_local;User Id=sa;Password=Aplicacionweb123!;TrustServerCertificate=True;" `
  -v "${PWD}/api_investigacion:/app" `
  -v /app/bin `
  -v /app/obj `
  -p 8070:8070 api-investigacion

# 6. Construir la imagen de front-blazor y encenderla
docker build -t front-blazor ./front_blazor
docker run -d --name investigacion-front --network proyecto_investigacion1_default --restart unless-stopped `
  -e "UrlApi=http://api-investigacion:8070" `
  -v "${PWD}/front_blazor:/app" `
  -v /app/bin `
  -v /app/obj `
  -p 8071:8071 front-blazor
...
```yaml
URL_API: http://api-investigacion:8070
```
...
|---|---|---|
| **Su navegador** | `http://localhost:8070` | Está fuera de Docker. Usa el puerto **publicado** |
| **Otro contenedor** | `http://api-investigacion:8070` | Están en la misma red: se llaman por el **nombre del servicio** |
...
> **En este proyecto los dos números son iguales** (`8070:8070`), y eso
...
Si alguien cambiara esa línea por `http://localhost:8070`, el front
...
```powershell
docker network inspect proyecto_investigacion1_default
```
...
`docker-compose.yml` y le pega `_default`. La carpeta `proyecto_investigacion1` produce
la red `proyecto_investigacion1_default`. Tres formas de confirmarlo:
...
docker inspect investigacion-front --format '{{range $k,$v := .NetworkSettings.Networks}}{{$k}}{{end}}'
```
...
network proyecto_investigacion1_default has active endpoints (name:"investigacion-front" …)
```
