...
│ Controllers/AreaConocimientoController.cs                   │
...
│ Servicios/ServicioAreaConocimiento.cs                       │
...
│ Repositorios/RepositorioAreaConocimientoSqlServer.cs        │
│ El SQL a mano (Dapper): traduce filas ↔ objetos     │
│ AreaConocimiento. NO conoce HTTP. NO decide negocio.        │
└────────────────┬────────────────────────────────────┘
                 │  ④ SELECT … FROM area_conocimiento WHERE codigo = @codigo
                 ▼
          ┌───────────────┐
          │ BASE DE DATOS │  SQL Server — investigacion_local
          └───────┬───────┘
                  │
...
|---|---|---|---|
| **Controller** | HTTP: rutas, códigos de estado, JSON | SQL y reglas de negocio | `Controllers/AreaConocimientoController.cs` |
| **Servicio** | Las reglas del negocio (¿existe? ¿se puede?) | Saber de HTTP o del motor de BD | `Servicios/ServicioAreaConocimiento.cs` |
| **Repositorio** | El SQL y el mapeo fila ↔ objeto | Saber de HTTP o decidir negocio | `Repositorios/RepositorioAreaConocimientoSqlServer.cs` |
...
Pregunta legítima: la carpeta de modelos (`Modelos/AreaConocimiento.cs`) existe en el
...
AGREGAR clases (`RepositorioAreaConocimientoPostgres : IRepositorioAreaConocimiento`) y
...
public class RepositorioAreaConocimientoPostgres : IRepositorioAreaConocimiento { /* … */ }
...
`RepositorioAreaConocimientoSqlServer` en las pruebas y el servicio ni se entera.

```csharp
// El repositorio FALSO de las pruebas (criterio 6): sin BD, misma interfaz
public class RepositorioFalsoEnMemoria : IRepositorioAreaConocimiento
{
    private readonly Dictionary<string, AreaConocimiento> _datos = new();

    public Task<AreaConocimiento?> ObtenerPorCodigoAsync(string codigo)
        => Task.FromResult(_datos.GetValueOrDefault(codigo));
    // ...los otros 4 métodos...
}

// y el servicio NI SE ENTERA:
var servicio = new ServicioAreaConocimiento(new RepositorioFalsoEnMemoria());
```

### I — Segregación de interfaces
Interfaces pequeñas y específicas: `IRepositorioAreaConocimiento` tiene SOLO los 5
métodos de datos de area_conocimiento — no un "IRepositorioDeTodo" que obligue a
implementar métodos que no se usan.

```csharp
// ✅ La interfaz de la v1: SOLO los 5 métodos de datos de area_conocimiento
public interface IRepositorioAreaConocimiento
{
    Task<List<AreaConocimiento>> ObtenerTodosAsync(int limite);
    Task<AreaConocimiento?> ObtenerPorCodigoAsync(string codigo);
    Task CrearAsync(AreaConocimiento area_conocimiento);
...
```csharp
public ServicioAreaConocimiento(IRepositorioAreaConocimiento repositorio)  // ← interfaz, no clase
```
...
```csharp
// El contrato (Repositorios/IRepositorioAreaConocimiento.cs):
Task<AreaConocimiento?> ObtenerPorCodigoAsync(string codigo);

// ServicioAreaConocimiento lo usa SIN saber si detrás hay SQL Server, otro motor
...
builder.Services.AddScoped<IRepositorioAreaConocimiento>(_ => fabrica.CrearRepositorioAreaConocimiento());
...
(`RepositorioAreaConocimientoSqlServer`, el falso de las pruebas — y los
