...
{
    conteo.TryGetValue(f.Gran_area, out var n);
    conteo[f.Gran_area] = n + 1;
}
```
...
Y en las capas: `RepositorioAreaConocimientoSqlServer` guarda su cadena de conexión
...
interfaz. `ServicioAreaConocimiento` funciona igual con
`RepositorioAreaConocimientoSqlServer` (el real) que con `RepositorioFalsoEnMemoria`
(el de la prueba de capas) — porque ambos firman `: IRepositorioAreaConocimiento`.
...
(`IServicioAreaConocimiento`, `IRepositorioAreaConocimiento`) son abstracción pura: declaran
...
classDiagram
    ...
    class RepositorioAreaConocimientoSqlServer {
    ...
    RepositorioAreaConocimientoSqlServer ..|> IRepositorioAreaConocimiento : POLIMORFISMO
    ...
    note for RepositorioAreaConocimientoSqlServer "ENCAPSULAMIENTO: la conexión y el SQL no salen de aquí"
```
...
{
    Task<bool> CrearAsync(AreaConocimiento datos);
}
...
{
    public Task<bool> CrearAsync(AreaConocimiento datos)
...
classDiagram
    ...
    ServicioMal --|> RepositorioAreaConocimientoSqlServer : hereda del CONCRETO — quedó casado con SQL Server
...
sequenceDiagram
    ...
    participant C as AreaConocimientoController
...
> modelos viven en `Modelos/`; en la v1 hay uno: `AreaConocimiento`.
...
| En la v1 | `AreaConocimiento` | `AreaConocimientoCrear`, `AreaConocimientoReemplazo`, `AreaConocimientoActualizar` |
...
body `{"stock": 99}` muere en 422 contra `AreaConocimientoReemplazo` y pasa
contra `AreaConocimientoActualizar`. Cuando todos los verbos comparten forma, la
...
- **Propiedades con `required`**: no se puede construir un `AreaConocimiento` sin
...
- **La petición declara y el framework valida**: las peticiones por verbo
  (`AreaConocimientoCrear`, `AreaConocimientoReemplazo`, `AreaConocimientoActualizar`) llevan sus
...
AreaConocimientoCrear / Reemplazo / Actualizar ← las PETICIONES: reglas por verbo (la frontera)
AreaConocimientoController               ← objeto HTTP; compone un IServicioAreaConocimiento
...
RepositorioAreaConocimientoSqlServer     ← implementación concreta (encapsula ADO.NET y SQL)
...
4. En este repositorio: [SOLID_CAPAS_PATRONES.md](SOLID_CAPAS_PATRONES.md) y el código
   de `api_investigacion/` (comentado línea a línea).
